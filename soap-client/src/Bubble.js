...
class Bubble {
  ...
  draw(q5) {
    ...
    // q5.image(shape, this.pos.x -shapeLength/2 , this.pos.y - shapeLength/2)
    ...
    for (let angle = 0; angle < q5.TWO_PI; angle += step) {
      ...
      if(angle ===  0){
        ctx.moveTo(shapeLength/2 + p.x, shapeLength/2 + p.y)
      }else{
        ctx.lineTo(shapeLength/2 + p.x, shapeLength/2 + p.y)
      }
    }
    ctx.closePath();
    ...
    // Draw clipped image
    ctx.clip()
    if (this.image) ctx.drawImage(this.image, 0,0, shapeLength, shapeLength)
